...
export function CharacterPage() {
  ...
  const [initialDiceLog, setInitialDiceLog] = useState<
    DiceLogEntry[] | undefined
  >(undefined);
  ...
  const diceLogSetterRef = useRef<((entries: DiceLogEntry[]) => void) | null>(
    null,
  );
    ...
    if (isApiCharacterId(id)) {
      Promise.all([
        ...
        .then(([charRes, stateRes]) => {
          ...
          const s =
            (stateRes as { state: Record<string, unknown> | null }).state ?? {};
  ...
  useCharacterRealtime(isApiChar ? id : undefined, {
    onCharacterUpdate: (data) => {
      ...
      if (id) {
        api.characters
          .get(id)
          .then((res) => {
            const raw = (res as { character: Record<string, unknown> })
              .character;
    ...
    onInventoryChange: async () => {
      if (!id) return;
      try {
        const res = (await api.inventory.get(id)) as {
          bags: ApiRawBag[];
          items: ApiRawItem[];
        };
  ...
  useEffect(() => {
    ...
    api.campaigns
      ...
      .then((res) =>
        setMembership(
          (res as { membership: CampaignMembership | null }).membership,
        ),
      )
        ...
        <div className="text-center space-y-4">
          ...
          <button
            onClick={() =>
              fromCampaignId
                ? navigate(
                    `/campanha/${fromCampaignId}${fromCampaignView ? `?view=${fromCampaignView}` : ""}`,
                  )
  ...
  return (
    <DiceLogProvider
      ...
      persistEntry={
        isApiChar && !!user && !!id
          ? (entry) => void api.state.appendDiceLog(id, entry)
          : undefined
      }
      persistClear={
        isApiChar && !!user && !!id
          ? () => void api.state.clearDiceLog(id)
          : undefined
      }
              ...
              {membership ? (
                ...
                >
                  <div>
                    ...
                    <button
                      onClick={() =>
                        navigate(`/campanha/${membership.campaignId}`)
                      }
                      style={{
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleJoinCampaign()
                      }
                    />
                    <button
