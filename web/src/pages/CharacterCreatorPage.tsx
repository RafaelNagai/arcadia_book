...
export function CharacterCreatorPage() {
  ...
  const localExisting =
    editId && !isApiCharacterId(editId)
      ? getCustomCharacter(editId)
      : undefined;
  const [apiExisting, setApiExisting] = useState<Character | undefined>(
    undefined,
  );
  const [apiLoading, setApiLoading] = useState(
    editId ? isApiCharacterId(editId) : false,
  );
  ...
  useEffect(() => {
    ...
    api.characters
      .get(editId)
      .then((res) => {
        const char = mapApiToCharacter(
          (res as { character: Record<string, unknown> }).character,
        );
        setApiExisting(char);
        setName(char.name);
        setRace(char.race);
  ...
  const [attrs, setAttrs] = useState<CharacterAttributes>(
    existing?.attributes ?? EMPTY_ATTRS,
  );
  const [skills, setSkills] = useState<CharacterSkills>(
    existing?.skills ?? EMPTY_SKILLS,
  );
  ...
  const [modificadores, setModificadores] = useState<CharacterModificadores>(
    existing?.modificadores ?? EMPTY_MODS,
  );
  ...
  useEffect(() => {
    document.title = isEditing
      ? "Editar Personagem — Arcádia"
      : "Criar Personagem — Arcádia";
  ...
  const handleArcanoChange = useCallback(
    (k: string, v: string | number | CharacterModificadores) => {
      ...
      else if (k === "modificadores")
        setModificadores(v as CharacterModificadores);
    },
    [],
  );
  ...
  async function handleSaveSection(_step: number): Promise<boolean> {
    ...
    try {
      ...
      const character: Character = {
        ...
        sanidade: newSan,
        currentHp: existing
          ? Math.min(existing.currentHp ?? existing.hp, newHp)
          : undefined,
        currentSanidade: existing
          ? Math.min(existing.currentSanidade ?? existing.sanidade, newSan)
          : undefined,
        owned: true,
      ...
      if (user) {
        ...
        const payload = mapCharacterToApi({
          ...character,
          image: isDataUrl ? null : image,
        });
        ...
        if (campaignId && !isEditing) {
          try {
            await api.campaigns.addNpc(campaignId, charId);
          } catch {
            /* ignore */
          }
  ...
  async function handleSave() {
    if (isSectionEdit) {
      const ok = await handleSaveSection(step);
      if (ok) navigate(-1);
      return;
    }
    await handleSaveSection(step);
  }

  async function handleSaveAndNavigate(targetStep: number) {
    const ok = await handleSaveSection(step);
    if (ok) {
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            ...
            {step === 2 && (
              <Step2Attrs
                attrs={attrs}
                onChange={(k, v) => setAttrs((prev) => ({ ...prev, [k]: v }))}
              />
            )}
            {step === 3 && (
              <Step3Skills
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          {isSectionEdit ? (
            ...
            >
              ...
              {step > 1 && (
                <button
                  onClick={() => handleSaveAndNavigate(step - 1)}
                  disabled={saving}
                  style={{
                    flex: "0 0 auto",
            ...
            >
              {step === 1 && !canProceed()
                ? "Preencha nome e raça para continuar"
                : "Continuar"}
            </button>
          ) : (
              ...
              <button
                ...
                style={{
                  ...
                  background: saving
                    ...
                    : "var(--color-arcano)",
                  cursor: saving ? "not-allowed" : "pointer",
                  color: saving ? "rgba(255,255,255,0.2)" : "#0A0A0A",
                  fontFamily: "var(--font-ui)",
                  fontSize: "0.8rem",
                  fontWeight: 700,
                  letterSpacing: "0.15em",
