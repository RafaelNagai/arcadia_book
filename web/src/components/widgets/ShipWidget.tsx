...
function StatBox({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="flex flex-col items-center justify-center py-3 px-2">
      ...
      <span className="font-display font-bold text-xl" style={{ color: '#C8E0F0' }}>
        {value}
      </span>
    </div>
  )
...
function SectorRow({ sector }: { sector: ShipSector }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2"
      ...
      {/* Category pill */}
      <span className="flex-shrink-0 text-xs px-2 py-0.5 rounded-sm mt-0.5"
        style={{
          background: `${color}22`,
          color,
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline justify-between gap-2">
          <span className="text-sm font-semibold" style={{ color: '#C8E0F0', fontFamily: 'var(--font-ui)' }}>
            {sector.name}
          </span>
          <span className="text-xs flex-shrink-0" style={{ color: 'var(--color-text-muted)', fontFamily: 'var(--font-ui)' }}>
            {sector.slots} slot{sector.slots > 1 ? 's' : ''}
          </span>
        </div>
        <div className="flex items-center gap-3 mt-0.5">
          <span className="text-xs" style={{ color: ACCENT_GLOW }}>{sector.effect}</span>
          {sector.test !== '—' && (
      ...
      <div className="px-6 py-5 relative overflow-hidden"
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="font-display text-2xl font-bold" style={{ color: '#C8E0F0', letterSpacing: '0.03em' }}>
              {ship.name}
            </h3>
          ...
          <div className="flex-shrink-0 text-right">
            ...
            <p className="text-xs mt-1" style={{ color: ACCENT_GLOW, fontFamily: 'var(--font-ui)' }}>
                ...
                return (
                  <li key={i} className="flex gap-2 text-sm">
                    ...
                    {rest.length > 0 && (
                      <>
                        <span style={{ color: ACCENT_DIM }}>—</span>
                        <span style={{ color: 'var(--color-text-secondary)' }}>{rest.join(' — ')}</span>
                      </>
                    )}
                  </li>
                )
...
export function ShipWidget() {
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="flex gap-1 mb-6 p-1 rounded-sm overflow-x-auto"
        style={{ background: 'rgba(32,96,160,0.08)', border: `1px solid ${ACCENT_DIM}` }}>
        {SHIPS.map((s, i) => (
          <button
            key={s.name}
