...
const ALL_TIERS = Array.from(new Set(EQUIPMENT.map((e) => e.tier))).sort(
  (a, b) =>
    Object.keys(TIER_COLOR).indexOf(a) - Object.keys(TIER_COLOR).indexOf(b),
);
        ...
        {item.effects.length > 0 ? (
          <ul className="space-y-1.5 flex-1">
            {item.effects.map((eff, i) => (
              ...
              >
                <span style={{ color: tierColor, flexShrink: 0, marginTop: 1 }}>
                  ◆
                </span>
                {eff}
              </li>
...
export function EquipmentWidget() {
  ...
  const groupedSections = useMemo<GroupedSection[]>(() => {
    ...
    for (const item of filtered) {
      const group = map.get(item.subcategory);
      if (group) {
        group.push(item);
      } else {
      ...
      <div className="space-y-5">
        ...
        <AnimatePresence mode="popLayout">
          {groupedSections.length > 0 ? (
            <motion.div key="sections" className="space-y-8">
              {groupedSections.map(({ subcategory, items }) => (
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div
                      style={{
                        ...
                        height: 1,
                        background:
                          "linear-gradient(to right, rgba(200,146,42,0.35), transparent)",
                      }}
                    />
                  </div>
