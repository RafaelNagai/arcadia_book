...
export class UploadService {
  ...
  async uploadMapLayerImage(
    ...
    const maxBytes = env.MAX_IMAGE_SIZE_MB * 1024 * 1024 * 4
    if (buffer.byteLength > maxBytes) {
      throw new ValidationError(`Imagem deve ter no máximo ${env.MAX_IMAGE_SIZE_MB * 4}MB`)
    }
    ...
    const ext = originalName.split('.').pop() ?? 'jpg'
    const path = `maps/${userId}/${mapId}/${Date.now()}.${ext}`
