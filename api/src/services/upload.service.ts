...
export class UploadService {
  ...
  async uploadShipImage(
    ...
    const ext = originalName.split('.').pop() ?? 'jpg'
    const path = `${userId}/${shipId}/${Date.now()}.${ext}`

    const { error } = await this.db.storage
      .from(env.SUPABASE_SHIP_BUCKET)
      .upload(path, buffer, { contentType: mimeType, upsert: true })

    if (error) throw new ValidationError(`Erro no upload: ${error.message}`)

    const { data } = this.db.storage.from(env.SUPABASE_SHIP_BUCKET).getPublicUrl(path)
    return data.publicUrl
  }

  async deleteShipFolder(userId: string, shipId: string): Promise<void> {
    const folder = `${userId}/${shipId}`
    const { data } = await this.db.storage.from(env.SUPABASE_SHIP_BUCKET).list(folder)
    if (!data || data.length === 0) return
    const paths = data.map(f => `${folder}/${f.name}`)
    await this.db.storage.from(env.SUPABASE_SHIP_BUCKET).remove(paths)
  }

  /** Deletes a file given its Supabase public URL. Tries both buckets automatically. No-op if URL doesn't match either. */
  async deleteImageByUrl(url: string): Promise<void> {
    if (!url) return
    for (const bucket of [env.SUPABASE_STORAGE_BUCKET, env.SUPABASE_MAP_STORAGE_BUCKET, env.SUPABASE_CREATURE_BUCKET, env.SUPABASE_CAMPAIGN_BUCKET, env.SUPABASE_SHIP_BUCKET]) {
