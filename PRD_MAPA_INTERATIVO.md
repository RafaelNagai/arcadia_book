...
> **Status:** Em desenvolvimento — Fases 1–4 + Polimentos + Multi-Floor concluídos  
> **Versão:** 1.6  
> **Data:** 2026-04-21  
...
- `MapTool` é `'select' | 'fog' | 'wall'`; pan + drag de token no mesmo modo
...
#### QoL de paredes
- Clicar fora de uma parede selecionada apenas deseleciona (não inicia nova construção)
- Círculos interativos nos endpoints da parede selecionada → clicar continua construindo a partir daquele ponto
...
selection
```
- Andares inferiores: imagem em 55% de opacidade para indicar profundidade
...
- Operações de tokens, paredes, fog e drag-drop sempre operam no andar atual do GM
...
5. **🌫 Névoa ON/OFF** → fog de guerra com LOS real
6. **Ferramenta Fog**: clique no canvas revela manualmente (usa LOS do ponto)
7. **↺ Reset névoa**: limpa exploração da layer atual
...
4. Áreas exploradas persistem; paredes bloqueiam LOS
...
### Objetivos
- Mestre controla totalmente o estado do mapa (tokens, fog, layers, andares)
...
|---|---|
| **Mestre** | Criar/editar/deletar mapas; mover tokens; paredes; fog; selecionar andar ativo; reordenar layers |
...
- [x] **RF-MF-03** — Andares inferiores em 55% de opacidade para indicar profundidade
...
| **RNF-04** | Fog calculado no frontend; paredes e posições são source of truth no servidor |
...
DELETE /campaigns/:id/maps/:mapId/layers/:lid/fog/patches   → resetar exploração

POST   /upload/map-layer                                    → upload PNG (até MAX_MAP_IMAGE_SIZE_MB = 30MB)
POST   /upload/character-image                              → upload foto (até MAX_IMAGE_SIZE_MB = 15MB)
```
...
| Regra | Onde |
|---|---|
| Apenas mestre cria/edita/deleta mapas, layers, walls, tokens | `MapsService.assertMapGm()` |
| Jogador só lê mapas de campanhas que participa | `MapsService.assertCampaignAccess()` |
| Fog write: apenas mestre | `assertMapGm()` |
| Resize, modal config, troca de andar: apenas mestre | UI (`isGm` guard) |
...
| Fog | **Ray Casting 2D** (`fogOfWar.ts`) | Funciona bem para N < 20 paredes por layer |
