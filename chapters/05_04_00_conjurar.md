...
### 2. O Elemento
O Mestre e Jogador decidem qual dos **Cinco Elementos** a magia pertence: *Energia, Anomalia, Paradoxo, Astral ou Cognitivo*. O personagem só pode conjurar com seu elemento de **Afinidade** ou **Antítese**.

### 3. Os Modificadores Exigidos
Definido qual o elemento, o Mestre determina quais modificadores a magia exige com base na intenção. Toda magia exige no mínimo 2 modificadores.

### 4. Distribuição da Pool de Dados
Todo personagem dispõe de **2D12** para distribuir entre os modificadores exigidos pela magia. Pode concentrar ambos em um único modificador — o outro usará apenas o valor do modificador.

- O jogador decide a distribuição **antes** de rolar
- Pode colocar ambos os dados em um único modificador — o outro modificador utilizará apenas valor + entropia, sem rolar um dado.
- Magias com 3 ou mais modificadores exigem Entropia alta para cobrir os modificadores sem dado disponível.

> Concentrar ambos os D12s em um modificador é a única forma de conseguir **Milagre** ou **Desastre** — e a única forma de aumentar o valor de um modificador.

Para cada modificador, o conjurador rola e soma:

```
Resultado = Valor do Modificador + D12s alocados + Bônus de Entropia
```

O resultado de cada modificador é lido na **Tabela de Grandeza** para determinar como aquela dimensão da magia se manifestou.

Entenda mais sobre a [ENTROPIA](entropia)
